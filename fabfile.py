...
def tail():
    sudo('tail -f /var/log/uwsgi/app/imprint75.log')
...
def build3():
    ...
    install_python()
    install_git()
    link_confs('imprint75', 'imprint75')
    create_env('imprint75')
    install_pip_reqs()
    restart()
...
def install_pip_reqs(venv='imprint75'):
    command = '/home/{0}/{1}/bin/pip install --upgrade -r ' \
              '/srv/{1}/{1}/requirements.txt'
    run(command.format(env.user, venv))


def link_confs(dir_name='imprint75', file_name='imprint75'):
...
def deploy():
    with cd('/srv/imprint75'):
        if env.user == 'imprint75':
            run('git pull')
